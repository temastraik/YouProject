...
$stmt->bindParam(':user_id', $_SESSION['user_id'], SQLITE3_INTEGER);
$user = $stmt->execute()->fetchArray(SQLITE3_ASSOC);

// Проверка роли (менеджеры и исполнители могут редактировать, но с разными правами)
if ($user['role'] !== 'manager' && $user['role'] !== 'executer') {
    die("Доступ запрещен");
}
